...
class SpamHam:
    ...
    def evaluate(self, words):
        ...
        for i in words:
            if i in self.spam and i in self.ham:
                R *= (self.spam[i]/75268) / (self.ham[i]/290673)

            if i in self.spam and i not in self.ham:
                R *= (self.spam[i]/75268) / (SMALL_NUMBER)

            if i in self.ham and i not in self.spam:
                R *= SMALL_NUMBER / (self.ham[i]/290673)
